classdef nanog_gata6_act_v2_model < models.nanog_gata6_2_model
    methods
        function obj = nanog_gata6_act_v2_model(varargin)
        ...
        function initialize(obj)
            ...
            obj.par = struct(...
               'alfa3',0.32,'alfa4',3.44,'alfa1',1.22,'alfa2',3.44,...
               'beta',1.43,'gamma',2.18,'delta',1.97,'eta',2.19,...
        ...
        function y = quasy_steady_state(obj, fgf)
            ...
            nanog = (fgf./(obj.par.alfa4-fgf)).^(1./obj.par.delta);
            gata6 = obj.par.alfa2.*nanog.^obj.par.gamma./(1+nanog.^obj.par.gamma);
            fgf4_ex = ((nanog-obj.par.alfa1.*gata6.^obj.par.beta./(1+gata6.^obj.par.beta))./(obj.par.alfa3-(nanog-obj.par.alfa1.*gata6.^obj.par.beta./(1+gata6.^obj.par.beta)))).^(1./obj.par.eta);
            y = [gata6; nanog; fgf; fgf4_ex];
        end
        
        function y = quasy_steady_state_input(obj, fgf_ex)
            % calculate quasy steady state values of Nanog and Gata6 and
            % extract the input signal FGFex from secreted FGF4 value;
            % otherwise there is no close-form solution (mutliple roots etc)
            function F = root(x_in, fgf_ex, par)
                nanog = x_in(1); gata6 = x_in(2);
                F(1) = par.alfa1.*gata6.^par.beta./(1+gata6.^par.beta) + par.alfa3.*fgf_ex.^par.eta./(1+fgf_ex.^par.eta) - nanog;
                F(2) = par.alfa2.*nanog.^par.gamma./(1+nanog.^par.gamma) - par.k_deg.*gata6;
            end
            fun = @(x_in) root(x_in, fgf_ex, obj.par);
            options = optimoptions('fsolve','Display','off');
            n_init = 10;
            x0 = 3.5*rand(n_init,2);
            x = zeros(size(x0));
            for i=1:n_init
                x(i,:) = fsolve(fun,x0(i,:),options);
            end
            nanog = x(:,1); gata6 = x(:,2);
            fgf = obj.par.alfa4.*nanog.^obj.par.delta./(1+nanog.^obj.par.delta);
            y = [gata6; nanog; fgf; fgf_ex];
        end
        ...
        function [dydt] = df_model(obj, t, y, t_vec, fgf4ext_vec, fgf4prod_inh_vec)
            ...
            d_gata6 = obj.par.alfa2.*nanog.^obj.par.gamma./(1+nanog.^obj.par.gamma) -obj.par.k_deg.*gata6;
            d_nanog = obj.par.alfa1.*gata6.^obj.par.beta./(1+gata6.^obj.par.beta) +obj.par.alfa3.*fgf4_per.^obj.par.eta./(1+fgf4_per.^obj.par.eta) -obj.par.k_deg.*nanog;
